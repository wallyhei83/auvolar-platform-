  ...
  try {
    ...
    switch (type) {
      ...
      case 'rfq':
        ...
        const itemsHtml = data.items?.map((item: { sku: string; quantity: number; notes?: string }) => 
          `<tr>
    ...
    const result = await resend.emails.send({
      from: 'Auvolar Website <onboarding@resend.dev>',
      to: CONTACT_EMAIL,
      replyTo: data.email,
      subject,
      html: htmlContent,
    ...
    if (data.email) {
      await resend.emails.send({
        ...
        to: data.email,
        subject: `Thank you for contacting Auvolar`,
        html: `
